export function setupProtection() {
  ...
  const obf = ["404", "firebase", "genai", "ai", "run.app", "onesignal", "vercel", "api key", "currentcolor"];
  ...
  // Dynamically load external privacy protect script
  const script = document.createElement("script");
  script.src = "https://unpkg.com/screenshot-privacy-protect/dist/privacy-protect.min.js";
  script.onload = () => {
    if ((window as any).PrivacyProtect) {
      (window as any).PrivacyProtect.enable({ feedback: false, blockPrint: true, watermarkText: "" });
    }
  };
